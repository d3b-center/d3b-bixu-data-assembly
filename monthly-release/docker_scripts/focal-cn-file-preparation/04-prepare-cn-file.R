...
#histologies_df <- read.table(opt$metadata,sep=',',header = TRUE, stringsAsFactors = FALSE)
histologies_df <- read_csv(opt$metadata)
...
for (i in 1:length(slice_vector)){
  ...
  cnv_no_xy_each <- cnv_df_each %>%
    ...
    #dplyr::filter((chr %in% c("chr1","chr2","chr3","chr4","chr5","chr6","chr7","chr8","chr9","chr10","chr11","chr12","chr13","chr14","chr15","chr16","chr17","chr18","chr19","chr20","chr21","chr22","chrM")))
  ...
  autosome_annotated_cn <- dplyr::bind_rows(autosome_annotated_cn, autosome_annotated_cn_each)
}

# Output file name
if (opt$runWXSonly){
  autosome_output_file <- paste0(opt$prefix, ".",  opt$filename_lead, "_wxs_autosomes.tsv.gz")
}else{
  autosome_output_file <- paste0(opt$prefix,  ".", opt$filename_lead, "_autosomes.tsv.gz")
}
...
if (xy_flag) {
  ...
  # Output file name
  if (opt$runWXSonly){
    sex_chrom_output_file <- paste0(opt$prefix, ".", opt$filename_lead, "_wxs_x_and_y.tsv.gz")
  }else{
    sex_chrom_output_file <- paste0(opt$prefix, ".", opt$filename_lead, "_x_and_y.tsv.gz")
  }
