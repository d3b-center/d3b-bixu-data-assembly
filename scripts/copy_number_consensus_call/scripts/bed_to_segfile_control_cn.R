...
cnvs <- cnvs %>%
  dplyr::mutate(cnvkit_df = purrr::map(cnvkit_CNVs, segstrings_to_df),
                ...
                copynum = ifelse(is.finite(freec_cn), # use freec_cn if available
                                 freec_cn, cnvkit_cn), #otherwise use cnvkit_cn
#                tumor_ploidy = purrr::map_dbl(freec_df, tumor_ploidy),
                num.mark = NA)
...
out_table <- dplyr::bind_rows(out_cnvs) %>%
